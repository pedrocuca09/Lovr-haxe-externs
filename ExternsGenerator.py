#!/usr/bin/env python3
"""
Improved LoVR Haxe Extern Generator
Generates Haxe externs for the LoVR engine from documentation and source code.
"""
...
class ExternsGenerator:
    def __init__(self):
        ...
        # Type mapping from LoVR types to Haxe types
        self.type_map = {
            "number": "Float",
            "string": "String",
            "boolean": "Bool",
    ...
    def generate_externs(self):
        ...
        print("Generating LoVR Haxe externs (Improved)...")
        ...
        if os.path.exists(module_path):
            for file in os.listdir(module_path):
                if file.endswith(".lua") and file != "init.lua":
                    method_name = file[:-4]
                    doc_path = os.path.join(module_path, file)
                    with open(doc_path, encoding="utf-8") as f:
                        doc_content = f.read()
                    args = self.parse_arguments_from_content(doc_content)
                    returns = self.parse_returns_from_content(doc_content)
                    content += self.generate_method_signature(method_name, args, returns) + "\n"
...
'''
    ...
    def parse_arguments_from_content(self, content: str) -> List[Dict[str, str]]:
        """Extrae todos los argumentos de la sección arguments = { ... } como lista."""
        arguments = []
        arg_match = re.search(r'arguments\s*=\s*\{(.*?)\}\s*,', content, re.DOTALL)
        if arg_match:
            arg_section = arg_match.group(1)
            # Busca todos los bloques { ... }
            blocks = re.findall(r'\{(.*?)\}', arg_section, re.DOTALL)
            for block in blocks:
                # Extrae name, type, default
    ...
    def generate_method_signature(self, method_name: str, arguments: List[Dict[str, str]], returns: List[Dict[str, str]]) -> str:
        """Genera la firma de método Haxe usando todos los argumentos extraídos."""
        param_list = []
        for arg in arguments:
            haxe_type = self.type_map.get(arg["type"], "Dynamic")
            if arg.get("default") not in [None, "nil"]:
                param_list.append(f"?{arg['name']}:{haxe_type}")
            else:
                param_list.append(f"{arg['name']}:{haxe_type}")
        return_type = "Void"
        if returns:
